...
-   문서(가이드): 통합 참고 문서는 `doc/theory/Scaled-cPIKAN_이론과_구현_가이드.md`를 확인하세요. 이론(핀/칸/체비쇼프)부터 구현/훈련/샘플링/평가까지 한 문서로 정리되어 있습니다.

-   `README.md`: 현재 읽고 있는 파일입니다! 프로젝트의 개요를 설명합니다.
-   `doc/`: 상세 문서가 포함된 디렉토리입니다.
    -   `theory/Scaled-cPIKAN_이론과_구현_가이드.md`: 통합 가이드(권장).
    -   `Technical Design Document...`: (레거시) 알고리즘과 구현 설계에 대한 심층 설명서입니다.
